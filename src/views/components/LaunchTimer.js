...
class LaunchTimer extends React.Component {
  constructor(props) {
    super(props);
    let distanceInSeconds = Math.floor(
      (this.props.nextLaunch.date - new Date()) / 1000
  ...
  componentDidMount = () => {
    this.timeInterval = setInterval(
      this.substractASecond,
      1000 // Update the timer every second
    );
    this.updateTimeInterval = setInterval(
      this.updateTimeInterval,
      60000 // Every minute update the timer, in case the user wasn't using the app for some time
    );
    if (this.props.nextLaunch.datePrecision !== "hour") {
      return;
    }
  ...
  render() {
    ...
    return (
      <div className="text-center pt-4 timer-container">
        <h3 className="text-center launch-timer-title mt-4">
          The next launch, "<span className="highlight">{launchName}</span>"
          takes off in:
        </h3>
